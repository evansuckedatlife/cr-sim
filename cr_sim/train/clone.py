...
class Demonstrations:
    ...
    def save(self, path: Path) -> None:
        ...
        np.savez_compressed(
            path, grid=self.grid, vector=self.vector, mask=self.mask,
...
def clone(net, data: Demonstrations, config: CloneConfig | None = None,
    ...
    for epoch in range(config.epochs):
        ...
        for start in range(0, len(shuffled), config.batch_size):
            ...
            policy_loss = nn.functional.cross_entropy(logits, action[batch])
        ...
        with torch.no_grad():
            ...
            agreement = (logits.argmax(dim=-1) == action[test_idx]).float().mean().item()
